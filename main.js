...
templs = {
  header: {
    ...
    search: {
      ...
      _appendBg: function () {
        var _this = this;
        $(document).find(".header").append(this.backgroundPage);
        var bg = $(document).find(".header").find(".header_bg.bg_search");
      ...
      open: function () {
        ...
        this.animate = true;
        $(document).find(".header").eq(0).toggleClass("searchOpen");
        $(document).find(".header_search_input").eq(0).focus();
        $(document).find(".header_search_wrapper").eq(0).toggleClass("open");
        var _this = this;
        setTimeout(function () {
          _this._appendBg();
      ...
      close: function () {
        ...
        var _this = this;
        this.animate = true;
        $(document).find(".header_search_wrapper").removeClass("open");
        this._removeBg(function () {
          $(document).find(".header_mobileMenu").animate({
            opacity: 1,
          });
          $(document).find(".header").toggleClass("searchOpen");
        });
      },
      events: function () {
        var _this = this;
            ...
            if ($(window).width() <= 1024) {
              ...
              if (!_this.animate) {
                if (_this.state === "opened") {
                  if ($(document).find(".header_mobileMenu").hasClass("open"))
                    _this.close();
                }
              }
            }
    ...
    burger: {
      ...
      _appendBg: function () {
        var _this = this;
        $(document).find(".header").find(".header_bg").remove();
        $(document).find(".header").append(this.backgroundPage);
        var bg = $(document).find(".header").find(".header_bg.bg_burger");
        bg.css("display", "block");
        bg.animate(
          {
            opacity: 1,
      ...
      open: function () {
        ...
        $(document).find(".header").addClass("burgerOpen");
        var _this = this;
      ...
      close: function () {
        ...
        this.menu.animate(
          ...
          function () {
            _this._removeBg();
            $(document).find(".header").removeClass("burgerOpen");
          }
        );
      },
      init: function () {
        ...
        this.language.init();
        $(document).find(".header_mobile_burger").off();
        this.events();
        this.animate = false;
        this.state = "closed";
        this.close();
    ...
    checkAnim: function () {
      ...
      if ($(document).find(".footer").length > 0) {
        if (
          ...
          $(document).find(".footer").offset().top + 100
        ) {
          _this.anim();
        }
      }
  ...
  gallery: {
    ...
    createGallery: function () {
      ...
      $(
        ".swiper-container.photorama-main.swiper-container-initialized.swiper-container-horizontal"
      ).append("<div class='swiper-big-befor'></div>");
      $(
        ".swiper-container.photorama-main.swiper-container-initialized.swiper-container-horizontal"
      ).prepend("<div class='swiper-big-after'></div>");
      ...
      _.galleryBig.on("click", function (e) {
        if ($(window).width() <= 768) {
          if ($(".popup_full .photorama-main").hasClass("big")) {
            $(".popup_full .photorama-main").css({
              height: "calc(100% - 72px)",
              "background-color": "neno",
            });
      ...
      _.galleryThumbs.on("slideChange", function () {
        if (
          $(".photorama-box").height() -
            (_.galleryThumbs.realIndex * _.galleryThumbs.height -
              -1 * parseInt($(".swiper-wrapper").css("top"))) <
          0
        ) {
          var top =
            parseInt($(".swiper-wrapper").css("top")) - _.galleryThumbs.height;
      ...
      _.galleryBig.on("slideChange", function () {
        ...
        $(".photorama-main .swiper-slide.video").find("iframe").remove();
        _.galleryThumbs.update();
        _.galleryBig.update();
      });
      ...
      if ($(window).width() <= 768) {
        if ($(".swiper-slide").length / 2 == 1) {
          $(
            ".swiper-container.photorama-main.swiper-container-initialized"
          ).css("height", "100%");
        }
      ...
      $("#photorama").bind("mousewheel", function (e) {
        if (ThumbsHovered) {
          ...
          var allheight =
            -1 * (countimage * step - $("#photorama").height() + 0);
          if (e.originalEvent.wheelDelta < 0) step = -1 * step;
          var oldtop = parseInt(
            $(".photorama-thumbs .swiper-wrapper").css("top")
          );
          var top = oldtop + step;
          if (top > 0) top = 0;
          else if (top < allheight) top = allheight;
          ...
          if (oldtop > top) {
            ...
            if (numberimage > countimage) numberimage = countimage;
          } else if (oldtop < top) {
            numberimage = numberimage - 1;
            if (numberimage < 0) numberimage = 0;
          }
      ...
      $(document).keyup(function (e) {
        ...
        if (
          ...
          numberimage >= 0 &&
          numberimage != _.galleryBig.realIndex
        ) {
  ...
  errorMess: {
    ...
    copyText: function () {
      ...
      $("#messError").find("[name=error]").val(selectionText);
      $("#messError").find("[name=place]").val(allTextInBlockText);
      $("#messError").find("[name=url]").val(url);
    },
    popupOpen: function () {
      this.copyText();
  ...
  main: {
    ...
    init: function () {
      if (
        ...
        $(".fpSlider_wrapper").find("audio").remove();
      } else {
        $(".fpSlider_wrapper")
          .find("audio")
          .eq(0)
          .attr(
            "src",
            $(".fpSlider_wrapper").find("audio").eq(0).attr("data-src")
          );
        // $(".fpSlider")
        //   .find("video")
        //   .each(function() {
    ...
    fpPhoto: {
      ...
      events: function () {
        ...
        $(document).on("click", ".fpPhoto_box", function () {
          $(this).parent().find(".js-popup").click();
        });
      },
      show: function (item) {
      ...
      events: function () {
        ...
        this.moveBar(
          _this.buts.eq(0).offset().left -
            _this.buts.eq(0).parent().offset().left,
    ...
    yaMapCreate: {
      ...
      create_full: function () {
        ...
        myMapFull = new ymaps.Map(
          "map",
          {
            center: [55.76, 59.64], //_this.center,//
            zoom: zoom,
            controls: [],
          },
          {
      ...
      init: function () {
        ...
        this.center =
          $(".map_data .placeMark").length > 0
            ? $(".map_data .placeMark").eq(0).attr("data-cords").split(",")
            : [57.446132, 55.233856];
      ...
      if (item.hasClass("open")) {
        item.height(
          item.find(".vacancy_title").height() +
            item.find(".vacancy_title").css("padding-top").replace("px", "") *
              2 +
            2
        );
        item.removeClass("open");
    ...
    events: function () {
      ...
      $(document).on("click", ".comment_item > .default-btn", function (e) {
        ...
        if ($(this).parent().find(">.form").length == 0) {
          $(".comment").find(".parsley-required").remove();
          $(".comment").find("*").removeClass("parsley-error");
          var answerId = $(this).parent().attr("data-answer-id");
          $(".comment").find(".form").find('[name="abswer_id"]').remove();
          $(".comment")
            .find(".form")
            .append(
              '<input name="abswer_id" type="hidden" value="' + answerId + '"/>'
            );
          var html = $(".comment").find(".form").eq(0)[0].outerHTML;
